...
import functools
import math
from model.sed_module_attention import ModifiedSpatialTransformer
...
class SeD_P(nn.Module):
    ...
    def forward(self, input, semantic):
        ...
        input = self.conv1(input)  # dimout=128
        se = self.att1(semantic, input)  # semantic: 1024*16*16, input: 128*64*64
        # se: 64*64*64, input: 128*64*64
        input = self.lrelu(self.conv11(torch.cat([input, se], dim=1)))  # dimout=128
