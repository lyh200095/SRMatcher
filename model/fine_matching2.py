...
class FineMatching2(nn.Module):
    ...
    def get_fine_match(self, conf_matrix, data, W):
        ...
        def kmnn(conf_matrix, K=2):
            _, indices_k_dim2 = conf_matrix.topk(k=K, dim=2, largest=True)
            mask_k_dim2 = torch.zeros_like(conf_matrix, dtype=torch.bool).scatter_(2, indices_k_dim2, True)

            # Get the indices of the Kth nearest neighbors along dim=1 (h0c * w0c)
            _, indices_k_dim1 = conf_matrix.topk(k=K, dim=1, largest=True)
            mask_k_dim1 = torch.zeros_like(conf_matrix, dtype=torch.bool).scatter_(1, indices_k_dim1, True)

            return mask_k_dim2 * mask_k_dim1

        # 2. mutual nearest
        if getattr(self, 'kmnn', None) is None:
            mask = mask \
                   * (conf_matrix == conf_matrix.max(dim=2, keepdim=True)[0]) \
                   * (conf_matrix == conf_matrix.max(dim=1, keepdim=True)[0])
        else:
            mask = mask * kmnn(conf_matrix, K=self.kmnn)
        ...
        def get_topk(conf, k=4, dim=1, largest=False):
            values, indices = torch.topk(conf, k=k, dim=dim, largest=largest)
            mask = torch.zeros_like(conf, dtype=torch.bool).scatter_(dim, indices, True)
            mask.scatter_(dim, indices, True)
            return mask

        # m1 = get_topk(conf_matrix.view(-1, W * W * W * W), largest=True, k=1, dim=1)
        # m1 = m1.view(-1, W * W, W * W)

        mask = mask * non_mask.view(-1, W*W, W*W)
        # mask = mask * m1

        fine_b_ids = data['b_ids'].unsqueeze(-1).unsqueeze(-1).repeat([1, W * W, W * W])
        coarse_mconf = data['mconf'].unsqueeze(-1).unsqueeze(-1).repeat([1, W * W, W * W])
        coarse_mconf = coarse_mconf[mask]
        ...
        # if len(mconf) >= 4000:
        #     _, indices = mconf2.topk(k=4000, dim=0, largest=True)
        #     mconf, fine_b_ids, mkpts0_f, mkpts1_f = mconf[indices], fine_b_ids[indices], mkpts0_f[indices], mkpts1_f[indices]
