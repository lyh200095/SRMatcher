...
class GeoLoss(nn.Module):
    ...
    def forward(self, data):
        ...
        if 'dino_mask_s8' in data:
            loss_dino = self.compute_coarse_loss(
                data['dino_mask_s8'],
                data['conf_matrix_gt'])
            loss_scalars.update({"loss_dino": loss_dino.clone().detach().cpu()})
        if 'dect_conf_matrix_ori' in data:
            loss_dect_ori = self.compute_coarse_loss(
                data['dect_conf_matrix_ori'],
                data['conf_matrix_gt'])
            loss_scalars.update({"loss_dect_ori": loss_dect_ori.clone().detach().cpu()})
