...
class MultiSceneDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.hpatch_dataset = HpatchesDataset(st=0, rank=self.rank, world_size=self.world_size)
    ...
    def _build_concat_dataset(
        ...
        FOLDER = os.path.join(data_root, 'phoenix/S6/zl548/MegaDepth_v1')
        for npz_name in tqdm(npz_names,
            ...
            if (p := os.path.join(FOLDER, npz_name.split('_')[0])) and os.path.exists(p):
                pass
            else:
                if 1 or self.rank == 0:
                    logger.warning('rank: {}, folder not exists: {}'.format(self.rank, p))
                continue
        ...
        print('rank: {}, len: {}'.format(self.rank, len(datasets)))
    ...
    def val_dataloader(self):
        ...
        if not isinstance(self.val_dataset, abc.Sequence):
            ...
            dataloader = DataLoader(self.val_dataset, sampler=sampler, **self.val_loader_params)

            return [dataloader, dataloader_hpatch]
        else:
            dataloaders = []
