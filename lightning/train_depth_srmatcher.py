...
from lightning_depth_srmatcher import PL_SRMatcher
...
def parse_args():
    ...
    parser.add_argument(
        # '--data_cfg_path', type=str, help='data loftr_config path', default='train_config/megadepth_test1500.py')
...
def main():
    ...
    model = PL_SRMatcher(default_config, profiler=profiler, pretrained_ckpt=args.ckpt_path)
    ...
    logger = TensorBoardLogger(
        ...
        version='train_mega_ours_im704_epoch60',
        name=args.exp_name, default_hp_metric=False)
    ...
    ckpt_path=('last' if os.path.exists(os.path.join(cktp_dir, 'last.ckpt')) else None)
    ...
    # trainer.validate(model, datamodule=data_module, ckpt_path=('last' if os.path.exists(os.path.join(cktp_dir, 'last.ckpt')) else None))
